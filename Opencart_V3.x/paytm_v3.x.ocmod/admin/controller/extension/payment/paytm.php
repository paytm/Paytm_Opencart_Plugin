	...
	*/
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(!PaytmHelper::validateCurl(PaytmHelper::getPaytmURL(PaytmConstants::ORDER_STATUS_URL, $this->request->post['payment_paytm_environment'],$this->config->get('payment_paytm_merchant_id')))){
	...
	*/
	public function savetxnstatus() {
		...
		if(!empty($this->request->post['paytm_order_id']) && PaytmConstants::SAVE_PAYTM_RESPONSE){
				...
				$retry = 1;
				do{
					$resParams = PaytmHelper::executecUrl(PaytmHelper::getPaytmURL(PaytmConstants::ORDER_STATUS_URL, $this->request->post['payment_paytm_environment'],$this->config->get('payment_paytm_merchant_id')), $reqParams);
					$retry++;

...
class PaytmPayment { 
	...
	static function encrypt_e($input, $ky) {
		...
		if(function_exists('openssl_encrypt')){
			...
			// echo "Input with Padding => ".bin2hex($input)."<br>";
			$td = mcrypt_module_open(MCRYPT_RIJNDAEL_128, '', 'cbc', '');
			mcrypt_generic_init($td, $key, self::$iv);
			$data = mcrypt_generic($td, $input);
			mcrypt_generic_deinit($td);
			mcrypt_module_close($td);
			// echo "<br/><br/>AES Encrypted Hex => ".strtoupper(bin2hex($data))."<br/><br/>";
	...
	static function getChecksumFromArray($arrayList, $key) {
		...
		// echo "Final Hash without Salt => ".$hash."<br/>";
		$hashString = $hash . $salt;
		// echo "Final Hash with Salt => ".$hashString."<br/>";
	...
	static function verifychecksum_e($arrayList, $key, $checksumvalue) {
		...
		$finalString = $str . "|" . $salt;
		// echo "Final String<br/>".$finalString."<br/><br/>";
		$website_hash = hash("sha256", $finalString);
		// echo "Hash String without Salt<br/>".$website_hash."<br/><br/>";
		$website_hash .= $salt;
		// echo "Hash String with Salt<br/>".$website_hash."<br/><br/>";
